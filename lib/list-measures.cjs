...
{
	if (region.cm.fi === region.c3.fi)
	{
		return 'on line ' + region.cm.fi;
	}
	return 'on lines ' + region.cm.fi + ' through ' + region.c3.fi;
}
...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aq, request.ab, true);
		} catch (e) {
...
var $author$project$Bin$ListMeasures$GotKnownMeasures = $elm$core$Basics$identity;
...
var $rtfeldman$elm_iso8601_date_strings$DeadEnds$deadEndToString = function (deadend) {
	return $rtfeldman$elm_iso8601_date_strings$DeadEnds$problemToString(deadend.ex) + (' at row ' + ($elm$core$String$fromInt(deadend.eE) + (', col ' + $elm$core$String$fromInt(deadend.dY))));
};
...
var $elm$parser$Parser$Advanced$DeadEnd = F4(
	function (row, col, problem, contextStack) {
		return {dY: col, e3: contextStack, ex: problem, eE: row};
	});
...
var $elm$parser$Parser$Advanced$fromState = F2(
	function (s, x) {
		return A2(
			...
			A4($elm$parser$Parser$Advanced$DeadEnd, s.eE, s.dY, x, s.l));
	});
		...
		while (true) {
			...
			if (_Utils_eq(newOffset, -1)) {
				return A3(
					...
					0,
					{dY: col, l: s0.l, n: s0.n, h: offset, eE: row, f: s0.f});
			} else {
...
var $elm$parser$Parser$Advanced$chompWhile = function (isGood) {
	return function (s) {
		return A5($elm$parser$Parser$Advanced$chompWhileHelp, isGood, s.h, s.eE, s.dY, s);
	};
};
	...
	function (isGood, expecting) {
		return function (s) {
			...
			return _Utils_eq(newOffset, -1) ? A2(
				...
				true,
				0,
				{dY: 1, l: s.l, n: s.n, h: s.h + 1, eE: s.eE + 1, f: s.f}) : A3(
				$elm$parser$Parser$Advanced$Good,
				true,
				0,
				{dY: s.dY + 1, l: s.l, n: s.n, h: newOffset, eE: s.eE, f: s.f}));
		};
	});
...
var $elm$parser$Parser$Advanced$token = function (_v0) {
	...
	return function (s) {
		var _v1 = A5($elm$parser$Parser$Advanced$isSubString, str, s.h, s.eE, s.dY, s.f);
		var newOffset = _v1.a;
		var newRow = _v1.b;
		var newCol = _v1.c;
		return _Utils_eq(newOffset, -1) ? A2(
			...
			progress,
			0,
			{dY: newCol, l: s.l, n: s.n, h: newOffset, eE: newRow, f: s.f});
	};
};
...
var $elm$parser$Parser$DeadEnd = F3(
	function (row, col, problem) {
		return {dY: col, ex: problem, eE: row};
	});
var $elm$parser$Parser$problemToDeadEnd = function (p) {
	return A3($elm$parser$Parser$DeadEnd, p.eE, p.dY, p.ex);
};
...
var $elm$parser$Parser$Advanced$run = F2(
	function (_v0, src) {
		var parse = _v0;
		var _v1 = parse(
			{dY: 1, l: _List_Nil, n: 1, h: 0, eE: 1, f: src});
		if (!_v1.$) {
			var value = _v1.b;
...
var $author$project$Conjure$Scalar$defaultCodecs = {
	at: {
		dd: A2($elm$json$Json$Decode$map, $elm$core$Basics$identity, $dillonkearns$elm_graphql$Graphql$Internal$Builder$Object$scalarDecoder),
		c2: function (_v0) {
			var raw = _v0;
...
var $author$project$CustomScalarCodecs$codecs = $author$project$Conjure$Scalar$defineCodecs(
	{
		at: $author$project$Conjure$Scalar$defaultCodecs.at,
		an: {dd: $rtfeldman$elm_iso8601_date_strings$Iso8601$decoder, c2: $rtfeldman$elm_iso8601_date_strings$Iso8601$encode},
		d: {
			dd: A2($elm$json$Json$Decode$map, $elm$core$Basics$identity, $elm$json$Json$Decode$string),
			c2: function (_v0) {
				var s = _v0;
				return $elm$json$Json$Encode$string(s);
			}
		},
		aT: {dd: $elm$json$Json$Decode$value, c2: $elm$core$Basics$identity}
	});
...
var $dillonkearns$elm_graphql$Graphql$Document$Hash$HashData = F4(
	function (shift, seed, hash, charsProcessed) {
		return {aS: charsProcessed, bt: hash, az: seed, cf: shift};
	});
...
var $dillonkearns$elm_graphql$Graphql$Document$Hash$finalize = function (data) {
	var acc = (!(!data.bt)) ? (data.az ^ A2(
		...
		A2(
			...
			15,
			A2($dillonkearns$elm_graphql$Graphql$Document$Hash$multiplyBy, $dillonkearns$elm_graphql$Graphql$Document$Hash$c1, data.bt)))) : data.az;
	var h0 = acc ^ data.aS;
...
var $dillonkearns$elm_graphql$Graphql$Document$Hash$hashFold = F2(
	function (c, data) {
		var res = data.bt | ((255 & $elm$core$Char$toCode(c)) << data.cf);
		var _v0 = data.cf;
		if (_v0 === 24) {
			return {
				aS: data.aS + 1,
				bt: 0,
				az: A2($dillonkearns$elm_graphql$Graphql$Document$Hash$mix, data.az, res),
				cf: 0
			};
		} else {
			return {aS: data.aS + 1, bt: res, az: data.az, cf: data.cf + 8};
		}
	});
	...
	return A2(
		...
		function () {
			if (!field.$) {
				...
				var typeString = field.a.fL;
				var fieldName = field.a.d4;
				var _arguments = field.b;
...
var $author$project$Conjure$Query$measures = F2(
	function (fillInOptionals____, object____) {
		var filledInOptionals____ = fillInOptionals____(
			{a: $dillonkearns$elm_graphql$Graphql$OptionalArgument$Absent, W: $dillonkearns$elm_graphql$Graphql$OptionalArgument$Absent});
		var optionalArgs____ = A2(
			...
			_List_fromArray(
				[
					...
					A3($dillonkearns$elm_graphql$Graphql$Internal$Builder$Argument$optional, 'nameCont', filledInOptionals____.W, $dillonkearns$elm_graphql$Graphql$Internal$Encode$string)
				]));
		return A4(
...
var $author$project$Conjure$Object$Measure$id = A4(
	...
	'id',
	_List_Nil,
	$author$project$Conjure$Scalar$unwrapCodecs($author$project$CustomScalarCodecs$codecs).d.dd);
...
var $author$project$Conjure$errorToString = function (e) {
	if (!e.$) {
		...
		switch (e.a.$) {
			...
			case 3:
				var _v3 = e.a;
				var statusCode = _v3.a.fF;
				var statusText = _v3.a.fG;
				return A2(
					$elm$core$String$join,
					'\n',
					_List_fromArray(
...
var $dillonkearns$elm_graphql$Graphql$Http$queryRequest = F2(
	function (baseUrl, query) {
		return {
			cX: baseUrl,
			de: A2($dillonkearns$elm_graphql$Graphql$Http$Query, $elm$core$Maybe$Nothing, query),
			a9: $dillonkearns$elm_graphql$Graphql$Document$decoder(query),
			J: _List_Nil,
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{eX: false, ao: r.ao, a9: r.a9, J: r.J, aq: r.aq, O: r.O, eQ: r.eQ, ab: r.ab}));
};
var $elm$http$Http$riskyRequest = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{eX: true, ao: r.ao, a9: r.a9, J: r.J, aq: r.aq, O: r.O, eQ: r.eQ, ab: r.ab}));
};
...
var $elm$regex$Regex$Match = F4(
	function (match, index, number, submatches) {
		return {fd: index, fk: match, X: number, fH: submatches};
	});
	...
	function (forceMethod, url, queryParams, maybeOperationName, queryDocument) {
		...
		if (_Utils_eq(
			...
			return {
				...
				aq: 1,
				ab: A2($dillonkearns$elm_graphql$Graphql$Http$QueryParams$urlWithQueryParams, queryParams, url)
			};
		} else {
			return {ao: $elm$http$Http$emptyBody, aq: 0, ab: urlForGetRequest};
		}
	});
...
var $dillonkearns$elm_graphql$Graphql$Http$GraphqlError$GraphqlError = F3(
	function (message, locations, details) {
		return {de: details, fj: locations, fl: message};
	});
...
var $dillonkearns$elm_graphql$Graphql$Http$toReadyRequest = function (_v0) {
	...
	if (!_v1.$) {
		...
		var queryRequestDetails = A5(
			...
			querySelectionSet);
		return {
			ao: queryRequestDetails.ao,
			dd: $dillonkearns$elm_graphql$Graphql$Http$decoderOrError(request.a9),
			J: request.J,
			aq: function () {
				var _v2 = queryRequestDetails.aq;
				if (!_v2) {
					return 'GET';
				} else {
					return 'Post';
		...
		return {
			...
			aq: 'POST',
			O: request.O,
			ab: A2($dillonkearns$elm_graphql$Graphql$Http$QueryParams$urlWithQueryParams, request.ay, request.cX)
		};
	}
};
...
var $author$project$Bin$ListMeasures$init = function (fs) {
	var flags = function (d) {
		return A2($elm$json$Json$Decode$decodeValue, d, fs);
	}(
	...
	return _Utils_Tuple2(
		...
		function () {
			if (!flags.$) {
				var pat = flags.a;
				return A2($author$project$Conjure$getMeasures, pat, $elm$core$Basics$identity);
			} else {
				var err = flags.a;
...
var $author$project$CustomScalarCodecs$viewId = function (_v0) {
	var s = _v0;
	return s;
};
var $author$project$Measure$view = function (_v0) {
	var id = _v0.a;
	var name = _v0.dE;
	return name + (' -- ' + $author$project$CustomScalarCodecs$viewId(id));
};
var $author$project$Bin$ListMeasures$update = F2(
	function (_v0, _v1) {
		var res = _v0;
		return _Utils_Tuple2(
			0,
			function () {
				if (!res.$) {
					if (!res.a.b) {
						return $author$project$Log$error('You don\'t have any Time Entry measures on Conjure!  Make one or more \"Time Entry\" measures here: https://conjure.so/measures/new');
					} else {
						var ms = res.a;
						return $author$project$Log$info(
							A2(
								$elm$core$String$join,
								'\n',
								A2($elm$core$List$map, $author$project$Measure$view, ms)));
					}
				} else {
					var e = res.a;
					return $author$project$Log$error('Fetching known measures from Conjure failed with the following error:\n\n' + e);
				}
			}());
	});
var $elm$core$Platform$worker = _Platform_worker;
var $author$project$Bin$ListMeasures$main = $elm$core$Platform$worker(
	{
		fe: $author$project$Bin$ListMeasures$init,
		fI: function (_v0) {
			return $elm$core$Platform$Sub$none;
		},
		fM: $author$project$Bin$ListMeasures$update
	});
_Platform_export({'Bin':{'ListMeasures':{'init':$author$project$Bin$ListMeasures$main($elm$json$Json$Decode$value)(0)}}});}(this));